...
public class Controller : MonoBehaviour
{
    ...
    void OnMouseUp()
    {
        ...
        else if (!tutorialScript.EstaExecutandoTutorial)
        {
    ...
    public void AddTransformacoesSlot(bool tutorial = false)
    {
        ...
        //int.TryParse(slot.Substring(slot.IndexOf("_") + 1), out val);

        //countTransformacoes = (val > 0) ? (val + 1).ToString() : "1";

        posicaoColliderDestino = GameObject.Find(slot);
    ...
    public void AddObjGrafico(bool tutorial = false)
    {
        ...
        //string countObjGrafico = string.Empty;
        //if (DropPeca.countObjetosGraficos > 0)
        //    countObjGrafico = DropPeca.countObjetosGraficos.ToString();
    ...
    public void ConfiguraPropriedadePeca(PropriedadePeca propPeca = null, PropriedadeCamera camProp = null)
    {
        if (Global.listaObjetos != null && Global.listaObjetos.Contains(gameObject))
        {
            ...
            {
                //abrePropriedade.GetComponent<PropRotacionarScript>().preencheCampos();
            }
            else if (Global.gameObjectName.Contains(Consts.Transladar))
            {
                //abrePropriedade.GetComponent<PropTransladarScript>().preencheCampos();
            }
            else if (Global.gameObjectName.Contains(Consts.Escalar))
            {
                //abrePropriedade.GetComponent<PropEscalarScript>().preencheCampos();
            }
            ...
            {
                abrePropriedade.GetComponent<PropCuboScript>().Inicializar();
            }
