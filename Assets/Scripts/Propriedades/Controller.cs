...
public class Controller : MonoBehaviour
{
    ...
    void OnMouseUp()
    {
        if (PodeEncaixar())
        {
            if (gameObject.name.Contains("Transladar") || gameObject.name.Contains("Escalar") || gameObject.name.Contains("Rotacionar"))
            {
    ...
    void ConfiguraIluminacao(string sinal, bool tutorial = false)
    {
        ...
        if (!tutorialScript.EstaExecutandoTutorial && (Global.listaEncaixes.ContainsKey(IlumPeca.name)))
        {
    ...
    void ZerarTransform(GameObject go)
    {
        if (go != null)
        {
            ...
            go.transform.localScale = !go.name.Equals("GameObjectCubo") && go.name.Equals("GameObjectPoligono") ? new Vector3(1, 1, 1) : new Vector3(1000, 1000, 1000);
        }
    }
    ...
    void AddGameObjectTree(string firstNameObject, string extensionName, bool tutorial = false)
    {
        ...
        if (forma.Contains(Consts.CUBO))
        {
            mainGameObject = "CuboAmbiente" + ((!tutorial) ? GetSlot() : "Tutorial");
        }
        else if (forma.Contains(Consts.POLIGONO))
        {
            mainGameObject = "PoligonoAmbiente" + ((!tutorial) ? GetSlot() : "Tutorial");
        }
    ...
    bool ContainObjetoAmb(Transform transformParent)
    {
        Transform objAmb = null;

        objAmb = transformParent.GetChild(0);

        if (objAmb != null)
        {
            return transformParent.GetChild(0).name.Contains("CuboAmbiente");
        }
    ...
    public void ConfiguraPropriedadePeca(PropriedadePeca propPeca = null, PropriedadeCamera camProp = null)
    {
        if (Global.listaEncaixes.ContainsKey(gameObject.name))
        {
            ...
            else if (Global.gameObjectName.Contains(Consts.CAMERA))
            {
                ...
                abrePropriedade.GetComponent<PropCameraScript>().UpdateProp();
    ...
    string GetSlot()
    {
        string formaSlot = Global.listaEncaixes[Util_VisEdu.GetPecaByName(gameObject.name)];
    ...
    {
        string nomeAmb = GetForma() + "AmbObject";
