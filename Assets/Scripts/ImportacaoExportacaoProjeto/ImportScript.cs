...
public class ImportScript : MonoBehaviour
{
        ...
        if (project.ObjetosGraficos?.Count > 0)
        {
            foreach (var obj in project.ObjetosGraficos)
            {
                ...
                if (obj.Forma.Propriedades.Nome.Contains("Cubo"))
                {
                    ...
                    cuboScript.ConfiguraPropriedadePeca(CreatePeca(obj.Forma.Propriedades));
                }
    ...
    PropriedadeCamera CreateCameraProp(PropriedadeCameraProject propProj)
    {
        return new PropriedadeCamera()
        {
            ...
            PropInicial = new PropriedadeCameraInicial()
            {
                PosX = DecodeField(propProj.PropInicial.PosX),
                PosY = DecodeField(propProj.PropInicial.PosY),
                PosZ = DecodeField(propProj.PropInicial.PosZ),
                FOV = propProj.PropInicial.FOV,
                ListPropCamInitLocks = new Dictionary<string, string>()
            },
    ...
    PropriedadePeca CreatePeca(PropriedadePecaProject propPecaProj)
    {
        return new PropriedadePeca()
        {
            ...
            NomeCuboAmbiente = propPecaProj.NomeCuboAmbiente,
            NomeCuboVis = propPecaProj.NomeCuboVis,
