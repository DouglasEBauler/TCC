...
public class PoligonoScript : MonoBehaviour
{
    ...
    public void AddPoligonoTransform()
    {
        if (!tutorialScript.EstaExecutandoTutorial)
        {
            ...
            GameObject poligonoAmb = GameObject.Find("PoligonoAmbiente" + GetNumeroSlotObjetoGrafico());
            if (poligonoAmb != null)
            {
        ...
        {
            GameObject.Find("PoligonoAmbiente" + GetNumeroSlotObjetoGrafico()).GetComponent<MeshRenderer>().enabled = true;
        }
    }
    ...
    void CreatePropPeca(PoligonoPropriedadePeca propPeca = null)
    {
        if (!Global.propriedadePecas.ContainsKey(gameObject.name))
        {
            ...
            if (Global.gameObjectName.Contains(Consts.POLIGONO))
            {
    ...
    public void ReposicionaSlots(string slotOrigem, string slotDestino)
    {
        ...
        if (slotOrigem.Contains("TransformacoesSlot") && getNumObjeto(slotOrigem).Equals(getNumObjeto(slotDestino)))
        {
            ...
            Global.listaEncaixes[gameObject.name] = slotDestino;
    ...
    public string GetNumeroSlotObjetoGrafico()
    {
        ...
        foreach (Transform child in render.transform)
        {
            if (child.name.Contains("ObjGraficoSlot")
                && (Global.listaPosicaoSlot.ContainsKey(child.name) && Global.listaPosicaoSlot.TryGetValue(child.name, out value))
    ...
    public void AtualizaTrasformGameObjectAmb(string GOname)
    {
        ...
        while (cont < breakLoop)
        {
            ...
            if (GoAmb.name.Contains("PoligonoAmbiente"))
            {
