...
public class DropPeca : MonoBehaviour
{
    ...
    {
        ProcessaExclusaoTransformacoesSlot();
        ProcessaExclusaoObjSlot();

        menuControl.GetComponent<MenuScript>().EnablePanelFabPecas();
        ReactiveColliderPeca();
    }

    void ProcessaExclusaoObjSlot()
    {
        if (Global.GetSlot(objDrop.name).Contains("ObjGraficoSlot"))
        {
    ...
    void ProcessaExclusaoTransformacoesSlot()
    {
    ...
    void ExcluiHierarquiaOuAlteraPropriedade(Slots tipoSlot)
    {
        ...
        switch (tipoSlot)
        {
            ...
            case Slots.FormasSlot:
                if (objDrop.name.Contains(Consts.CUBO))
                {
                ...
                else if (objDrop.name.Contains(Consts.POLIGONO))
                {
                    mr = GameObject.Find("PoligonoAmbiente" + getNumObjeto(GameObject.Find(Global.listaEncaixes[objDrop.name]).transform.parent.name)).GetComponent<MeshRenderer>();
                    mr.enabled = false;
                }
                else if (objDrop.name.Contains(Consts.SPLINE))
                {
                    mr = GameObject.Find("SplineAmbiente" + getNumObjeto(GameObject.Find(Global.listaEncaixes[objDrop.name]).transform.parent.name)).GetComponent<MeshRenderer>();
                    mr.enabled = false;
                }
        ...
        while (cont < breakLoop)
        {
            ...
            if (GoAmb.name.Contains("CuboAmbiente"))
            {
                ...
                if (Global.propriedadePecas.Count > 0)
                {
                    ...
                    {
                        if (enc.Value.Equals("FormasSlot" + getNumeroSlotObjetoGrafico()) && Global.propriedadePecas.ContainsKey(enc.Key))
                        {
    ...
    void configuraIluminacao(string sinal)
    {
        ...
        if (!tutorial.GetComponent<Tutorial>().EstaExecutandoTutorial && Global.listaObjetos.Contains(IlumPeca))
        {
